...
- (IBAction)onPayButtonClicked:(id)sender {
    ...
    NSInteger index = [self checkCoupons:_serviceSelectedIndexPath.row];
    if (index >= 0 && index < self.coupons.count) {
        MyCouponModel *model = self.coupons[index];
        paymentVC.couponID = model.couponID;
    }
    [UIApplication stopBusyHUD];
...
- (NSInteger)checkCoupons:(NSInteger)row {
    ...
    for (int i = 0; i < self.coupons.count; i++) {
        ...
        MyCouponModel *newModel = self.coupons[i];
        // 在券有效的前提下，优先减最多的（开始时间小于当前时间，结束时间大于当前时间）
        if (currrentTime < newModel.endTime && amount > newModel.limitPrice && amount >= [newModel.price floatValue]) {
            price = [newModel.price floatValue];
        }
